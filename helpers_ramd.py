...
def plot_init_part_ln_ln(force_value, n, t, t0, tend, output_file=None):
    pinit = np.polyfit(np.log(t[t0:tend]), np.log(-n[t0:tend]), 1)
    p = pinit[0]

    # plt.figure()
    # plt.plot(np.log(t[t0:tend]), np.log(-n[t0:tend]), 'o', color='magenta', linewidth=1.5)
    # plt.plot(np.log(t[t0:tend]), np.polyval(pinit, np.log(t[t0:tend])), '--', color='red', linewidth=1.5)
...
def plot_lnN(force_value, t, t_fit, n, tau_frac, pend, pinit, output_file=None):
    ...
    plt.plot(t[t < tau_frac], -np.exp(np.polyval(pinit, np.log(t[t < tau_frac]))), '--', color='black', linewidth=1.5, label='Exponential Fit')
